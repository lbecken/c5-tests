...
import type { ConflictResponse } from '@gitscope/server/protocol';

import { CommitCard } from '../commit/CommitCard';
import { DiffView } from '../diff/DiffView';
import { highlightLines, type Token } from '../../lib/highlight';

export type Choice = 'ours' | 'theirs' | 'ours-theirs' | 'theirs-ours' | 'base' | 'custom';
...
interface Props {
  ...
  /** Receives the fully resolved file contents. */
  onSave: (content: string) => Promise<{ ok: boolean; message: string }>;
  saveLabel?: string;
  /** Rendered next to the save button, e.g. a "cancel merge" action. */
  extraActions?: React.ReactNode;
}

/**
 * The conflict resolution surface, shared by repository conflicts and by files
 * handed over from `git mergetool`.
 *
 * The three stages sit side by side, but the part that actually resolves a
 * conflict is the context above them: which commits each side contributed since
 * the merge base, and what each side did to the *same* base text. A conflict is
 * a disagreement between two intentions, and you cannot pick a winner without
 * seeing both.
 */
export function MergeResolver({ data, onSave, saveLabel, extraActions }: Props) {
  ...
  // New file, new region indices: carrying choices across would silently apply
  ...
  const language = useMemo(() => guessLanguage(data.path), [data.path]);
  ...
  const save = async (): Promise<void> => {
    ...
    setSaveError(undefined);
    try {
      const result = await onSave(resolvedText);
    ...
    <>
      <div className="merge-toolbar">
        ...
        <button
          ...
          title={unresolved > 0 ? 'Resolve every conflict first' : 'Write the resolved file'}
        >
          {saving ? 'Saving…' : (saveLabel ?? 'Save and mark resolved')}
        </button>
      </div>
        ...
        <div className="merge-context">
          <section>
            ...
            {data.oursCommits.length === 0 ? (
              <p className="text-muted">No commit history available for this side.</p>
            ) : (
          ...
          <section>
            ...
            {data.theirsCommits.length === 0 ? (
              ...
              data.theirsCommits.map((entry) => <CommitCard key={entry.oid} commit={entry} compact />)
            )}
            <button
              type="button"
              className="link"
      ...
      {sideDiff !== 'none' ? (
        <div className="merge-side-diff">
          <DiffView
            diff={buildTextDiff(data.base, sideDiff === 'ours' ? data.ours : data.theirs, {
              context: 3,
            })}
            language={language}
            labels={{
...
function renderTokens(line: string, tokens: Token[] | undefined): React.ReactNode {
